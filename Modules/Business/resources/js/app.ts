...
createInertiaApp({
  ...
  // For the Business module, we need to make sure pages are correctly resolved
  resolve: (name) => {
    // If the page name is just 'Index', 'Show', etc., prepend 'business/'
    // If it already has a prefix (like 'business/Index'), leave it as is
    const pageName = name.includes('/') ? name : `business/${name}`;
    console.log(`Business module resolving: ${pageName}`);
